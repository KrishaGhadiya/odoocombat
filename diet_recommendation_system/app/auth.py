...
def signUp():
    if request.method == "POST":
        ...
        if user:
            ...
            new_user = User(email = email, password = generate_password_hash(password1, method='pbkdf2:sha256', salt_length=8), recommended_intake = recommended_dict["cals"], recommended_fats = recommended_dict["fats"], recommended_protein = recommended_dict["protein"], recommended_carbs = recommended_dict["carbs"], sex=sex, weight=weight, height=height, goal=goal, activity_factor=activity_factor,age=age, first_name=first_name)
            # add user to db
